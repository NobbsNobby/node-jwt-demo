...
test('User receives 403 with invalid credentials', async (t) => {
    const res = await app.post('/auth/login').send({
...
test('User receives 401 with expired token', async (t) => {
    const expiredToken = `Bearer ${issueToken({id:1}, {expiresIn: '1ms'})}`
    const res = await app.get('/users').set('Authorization', expiredToken)
    t.is(res.status,401);
})

test('User get new access token using refresh token', async (t) => {
...
test('User get 404 if refresh token is invalid', async (t) => {
    const res = await app.post('/auth/refresh').send({
...
test('Refresh tokens become invalid after logout', async (t) => {
    const logoutResponse = await app.post('/auth/logout').set('Authorization', `Bearer ${issueToken({id:2})}`);

    t.is(logoutResponse.status, 200);
